...
#define ISP_SCK_KEEP  0
#define ISP_SCK_SLOW  1
#define ISP_SCK_OK    2
#define ISP_SCK_FAST  3
...
static uchar flag_try_slow=0;
static uchar flag_tryed=0;
...
uchar usbFunctionSetup(uchar data[8]) {
	...
	if (data[1] == USBASP_FUNC_CONNECT) {
		...
		/* set SCK speed */
		if (((PINC & (1 << PC2)) == 0) || flag_try_slow==1) {
        ...
        if((replyBuffer[0]==1)&&(flag_try_slow==0)){
            flag_try_slow=1;
	...
	usbMsgPtr = replyBuffer;
        if ((data[1] != USBASP_FUNC_ENABLEPROG)&&
          (data[1] != USBASP_FUNC_CONNECT)&&
          (data[1] != USBASP_FUNC_DISCONNECT))flag_tryed=1;
	return len;
}
